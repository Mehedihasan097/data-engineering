    ...
    try:
        ...
        while True:
            ...
            SELECT 
                ...
                modified_by,
                owner
            from erpnext_prod.`tabSales Invoice`
            ...
            try:
                ...
                upsert_query = f"""
                INSERT INTO warehouse_dev.sales_invoice_table ({', '.join(mysql_data.columns)})
                VALUES %s
                ...
                if yesterday:
                    yesterday_sql = f"""
                    SELECT COUNT(*) FROM warehouse_dev.sales_invoice_table
        ...
        params = ('sales_invoice_table', start_date_time, end_date_time, total_rows, last_day_rows, status)
...
dag = DAG(
    'tab_Sales_Invoice_ETL_Incremental',
    default_args=default_args,
    description='Incremental load of sales invoice data from MySQL to PostgreSQL',
    schedule_interval='40 10 * * *',  # Daily at 10:40 AM
    catchup=False,
)

# Define the PythonOperator
transfer_task = PythonOperator(
    task_id='transfer_tab_sales_invoice_data_task_incremental',
