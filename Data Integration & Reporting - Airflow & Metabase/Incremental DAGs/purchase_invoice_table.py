    ...
    try:
        ...
        while True:
            ...
            SELECT 
                ...
                modified_by,
                owner
            from erpnext_prod.`tabPurchase Invoice`
            ...
            try:
                ...
                upsert_query = f"""
                INSERT INTO warehouse_dev.purchase_invoice_table ({', '.join(mysql_data.columns)})
                VALUES %s
                ...
                if yesterday:
                    yesterday_sql = f"""
                    SELECT COUNT(*) FROM warehouse_dev.purchase_invoice_table
        ...
        params = ('purchase_invoice_table', start_date_time, end_date_time, total_rows, last_day_rows, status)
...
dag = DAG(
    'tab_Purchase_Invoice_ETL_Incremental',
    default_args=default_args,
    description='Incremental load of purchase invoice data from MySQL to PostgreSQL',
...
transfer_task = PythonOperator(
    task_id='transfer_tab_purchase_invoice_data_task_incremental',
