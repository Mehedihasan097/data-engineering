    ...
    try:
        ...
        while True:
            ...
            SELECT 
                ...
                modified_by,
                owner
            from erpnext_prod.`tabItem Price`
            WHERE 1=1
            AND   DATE(creation) BETWEEN '{start_date}' AND '{end_date}'
            ...
            try:
                ...
                upsert_query = f"""
                INSERT INTO warehouse_dev.item_price_table ({', '.join(mysql_data.columns)})
                VALUES %s
                ...
                if yesterday:
                    yesterday_sql = f"""
                    SELECT COUNT(*) FROM warehouse_dev.item_price_table
        ...
        params = ('item_price_table', start_date_time, end_date_time, total_rows, last_day_rows, status)
...
dag = DAG(
    'tab_Item_Price_ETL_Incremental',
    default_args=default_args,
    description='Incremental load of item price data from MySQL to PostgreSQL',
...
transfer_task = PythonOperator(
    task_id='transfer_tab_item_price_data_task_incremental',
