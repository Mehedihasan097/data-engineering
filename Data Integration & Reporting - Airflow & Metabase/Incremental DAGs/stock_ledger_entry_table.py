    ...
    try:
        ...
        while True:
            ...
            SELECT 
                ...
                modified_by,
                owner
            from erpnext_prod.`tabStock Ledger Entry`
            ...
            try:
                ...
                upsert_query = f"""
                INSERT INTO warehouse_dev.stock_ledger_entry_table ({', '.join(mysql_data.columns)})
                VALUES %s
                ...
                if yesterday:
                    yesterday_sql = f"""
                    SELECT COUNT(*) FROM warehouse_dev.stock_ledger_entry_table
        ...
        params = ('stock_ledger_entry_table', start_date_time, end_date_time, total_rows, last_day_rows, status)
...
dag = DAG(
    'tab_Stock_Ledger_ETL_Incremental',
    default_args=default_args,
    description='Incremental load of stock ledger entry data from MySQL to PostgreSQL',
    schedule_interval='30 10 * * *',  # Daily at 10:30 AM
    catchup=False,
)

# Define the PythonOperator
transfer_task = PythonOperator(
    task_id='transfer_tab_stock_ledger_entry_data_task_incremental',
