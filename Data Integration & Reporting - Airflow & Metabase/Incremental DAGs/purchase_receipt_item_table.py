    ...
    try:
        ...
        while True:
            ...
            SELECT 
                ...
                modified_by,
                owner
            from erpnext_prod.`tabPurchase Receipt Item`
            ...
            try:
                ...
                upsert_query = f"""
                INSERT INTO warehouse_dev.purchase_order_receive_item_table ({', '.join(mysql_data.columns)})
                VALUES %s
                ...
                if yesterday:
                    yesterday_sql = f"""
                    SELECT COUNT(*) FROM warehouse_dev.purchase_order_receive_item_table
        ...
        params = ('purchase_order_receive_item_table', start_date_time, end_date_time, total_rows, last_day_rows, status)
...
dag = DAG(
    'tab_Purchase_Receipt_Item_ETL_Incremental',
    default_args=default_args,
    description='Incremental load of purchase receipt item data from MySQL to PostgreSQL',
    schedule_interval='37 10 * * *',  # Daily at 10:35 AM
    catchup=False,
)

# Define the PythonOperator
transfer_task = PythonOperator(
    task_id='transfer_tab_purchase_receipt_item_data_task_incremental',
