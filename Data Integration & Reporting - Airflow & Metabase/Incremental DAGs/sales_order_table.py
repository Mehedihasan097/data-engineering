    ...
    try:
        ...
        while True:
            ...
            SELECT 
                ...
                modified_by,
                owner
            from erpnext_prod.`tabSales Order`
            ...
            try:
                ...
                upsert_query = f"""
                INSERT INTO warehouse_dev.sales_order_table ({', '.join(mysql_data.columns)})
                VALUES %s
                ...
                if yesterday:
                    yesterday_sql = f"""
                    SELECT COUNT(*) FROM warehouse_dev.sales_order_table
        ...
        params = ('sales_order_table', start_date_time, end_date_time, total_rows, last_day_rows, status)
...
dag = DAG(
    'tab_Sales_Order_ETL_Incremental',
    default_args=default_args,
    description='Incremental load of sales order data from MySQL to PostgreSQL',
...
transfer_task = PythonOperator(
    task_id='transfer_tab_sales_order_data_task_incremental',
