...
def transfer_data(**context):
    try:
        ...
        while True:
            ...
            sql_query = f"""
            SELECT 
                name, creation, modified, parent, parentfield, parenttype, item_code,
                ifw_retailskusuffix, barcode, item_group, brand, description, image,
                warehouse, actual_qty, stock_qty, qty, delivered_qty, base_price_list_rate,
                price_list_rate, rate, base_rate, base_net_rate, net_rate, discount_percentage,
                discount_amount, base_net_amount, net_amount, base_amount, amount, margin_rate_or_amount,
                stock_uom_rate, incoming_rate, grant_commission, is_free_item, delivered_by_supplier,
                cost_center, expense_account, income_account, stock_uom, uom, target_warehouse,
                delivery_note, conversion_factor, sales_order, docstatus, idx, modified_by, owner
            FROM erpnext_prod.`tabSales Invoice Item`
            ...
            try:
                ...
                upsert_query = f"""
                INSERT INTO warehouse_dev.sales_invoice_item_table ({', '.join(mysql_data.columns)})
                VALUES %s
                ...
                if yesterday:
                    yesterday_sql = f"""
                    SELECT COUNT(*) FROM warehouse_dev.sales_invoice_item_table
        ...
        params = ('sales_invoice_item_table', start_date_time, end_date_time, total_rows, last_day_rows, status)
...
dag = DAG(
    'tab_Sales_Invoice_Item_ETL_Incremental',
    default_args=default_args,
    description='Incremental load of sales invoice item data from MySQL to PostgreSQL',
    schedule_interval='45 10 * * *',  # Daily at 10:45 AM
    catchup=False,
)

# Define the PythonOperator
transfer_task = PythonOperator(
    task_id='transfer_tab_sales_invoice_item_data_task_incremental',
