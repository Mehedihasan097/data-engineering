...
def generate_last_12_months_view(**kwargs):
    ...
    months = [(today - timedelta(days=30 * i)).strftime("%b_%Y_Receive") for i in range(12)]
    ...
    dynamic_sql = f"""
        CREATE OR REPLACE VIEW last_12_months_received AS
        SELECT
            c.item_code,
            {', '.join(case_statements)}
        FROM
            warehouse_dev.purchase_receive_item_table c
        LEFT JOIN
            warehouse_dev.purchase_receipt_table p ON p.name = c.parent
        WHERE
            p.docstatus = 1
